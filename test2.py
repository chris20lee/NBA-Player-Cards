...
interest_year = data[data['year'] == year]
players = interest_year['player_id'].unique()
num_players = len(players)

for ele, player in enumerate(players):
    ...
    try:
        ...
        plt.savefig('{}/Player Cards/{}_{}_{}_{}'.format(DATA_DIR, player_team, player_name, year - 1, year), dpi=2000)
        # plt.show()
        plt.close('all')

        print('{}/{}: {} ({}) player card for {}-{} season is finished'.format(ele + 1, num_players, player_name, player_team, year - 1, year))

    except:
        print('{}/{}: {} ({}) player card for {}-{} season could not be created'.format(ele + 1, num_players, player_name, player_team, year - 1, year))
        pass